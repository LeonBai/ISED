 ### The script hub for alternative embedding approachs that are mainly taken from third-party library
### It includes:
### 1. pca, t-sne (sklearn-tsne and openTSNE); umap, GP-LVM(old and new), T-PHATE, Cebra-time
### Note here: due to contrasting python enviroment for LFADS and MARBLE implementation, mainly due to pytorch versions and numpy/sklearn versions.
### We do not include LFADS and Marble here. 
### LFADS and MARBLE implementations can be found in respective notebooks. 
...
##### All set-ups and hyperparameters are used in simulation data (Fig. 2 main text)
...
def perform_pca_smooth(train_data, test_data, n_components=2, ar_order=150):  ## this ar_order can be tuned here 
    """
...
def train_ae_rnn(train_data, latent_dim=10, timesteps=500, input_dim=100, epochs=10000, batch_size=100):
    """
...
def perform_cebra(train_data, test_data, output_dimension=2, max_iterations=1000, batch_size=32, learning_rate=3e-4):
    ...
    """
    cebra_model = CEBRA(
        model_architecture='offset10-model', ### This can be changed with different pre-defined offset-value 
